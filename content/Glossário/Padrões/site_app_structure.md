...
## Descrição Geral
O padrão de estrutura do site/aplicativo refere-se à organização geral e arquitetura da informação de um portal ou aplicação. Envolve como o conteúdo e as funcionalidades são agrupados, interligados e apresentados ao usuário através da navegação principal, [[Componentes/menu|menus]], hierarquia de páginas e fluxos de usuário. Uma estrutura bem pensada é fundamental para a usabilidade, encontrabilidade (findability) e compreensão do sistema pelo usuário.

Este é um padrão de alto nível que influencia muitos outros padrões de UI (como navegação, [[Componentes/menu|menus]], [[Componentes/breadcrumbs|breadcrumbs]]) e está intrinsecamente ligado à Arquitetura da Informação (AI).
...
- **Navegação Principal (Primary Navigation):** [[Componentes/menu|Menu]] principal que dá acesso às seções de mais alto nível (ex: [[Elementos/cabecalhos|cabeçalho]], [[Componentes/menu|menu]] lateral).
- **Navegação Secundária/Local (Secondary/[[Componentes/local_navigation|Local Navigation]]):** [[Componentes/menu|Menus]] específicos para seções ou subseções.
- **[[Componentes/maps|Mapas]] do Site (Sitemaps):** Representação visual ou textual da hierarquia do conteúdo.
- **[[Componentes/breadcrumbs|Breadcrumbs]]:** Indicam a localização atual do usuário na hierarquia do site.
- **Página Inicial (Homepage/[[Entregáveis/dashboard|Dashboard]]):** Ponto de partida principal, oferecendo acesso a seções chave e conteúdo em destaque.
- **[[Componentes/footer|Rodapé]] ([[Componentes/footer|Footer]] Navigation):** [[Elementos/links|Links]] para informações secundárias, políticas, contato, etc.
...
2. O usuário utiliza os elementos de navegação ([[Componentes/menu|menus]], [[Elementos/links|links]], busca) para explorar o conteúdo e acessar funcionalidades.
3. A estrutura guia o usuário através de diferentes seções e níveis de informação.
4. [[Componentes/breadcrumbs|Breadcrumbs]] e outros sinais de orientação ajudam o usuário a manter o contexto.
5. Fluxos de usuário bem definidos conduzem a tarefas comuns de forma eficiente.

## Diretrizes de Uso e Boas Práticas

### Faça
- Baseie a estrutura nos modelos mentais e necessidades dos seus usuários (resultado de pesquisa com usuários, [[Componentes/cards|card]] sorting, tree testing).
...
- Não sobrecarregue os [[Componentes/menu|menus]] de navegação com muitas opções.
...
- Forneça um [[Elementos/links|link]] "Pular para o conteúdo principal" para usuários de teclado e leitores de tela.
- Certifique-se de que os [[Elementos/links|links]] tenham texto descritivo que faça sentido fora de contexto.
- [[Componentes/breadcrumbs|Breadcrumbs]] devem ser implementados de forma acessível.
- O [[Entregáveis/site_map|mapa do site]], se fornecido, deve ser acessível.

## Exemplos / Cenários de Uso
- Estrutura de um portal de e-commerce (categorias de produtos, conta do usuário, carrinho, [[Padrões/purchase_checkout|checkout]]).
- Estrutura de um portal de notícias (seções temáticas, artigos, busca, arquivos).
- Arquitetura de um aplicativo SaaS ([[Entregáveis/dashboard|dashboard]], módulos de funcionalidades, [[Padrões/settings|configurações]], ajuda).
...
- **Sequencial:** Informação apresentada em uma ordem específica, passo a passo (ex: tutoriais, processos de [[Padrões/purchase_checkout|checkout]]).
- **Matricial:** Permite que os usuários naveguem por múltiplos atributos ou facetas (comum em [[Componentes/filters|filtros]] de busca).
- **Orgânica/Rede:** Conteúdo interligado de forma mais fluida, com muitos [[Elementos/links|links]] cruzados (ex: wikis).
...
- [[[Elementos/links|Link]] para o [[Entregáveis/site_map|mapa do site]] ou diagramas da arquitetura da informação]
- [[[Elementos/links|Link]] para [[Entregáveis/wireframe|wireframes]] ou [[Entregáveis/prototype|protótipos]] mostrando a navegação principal e fluxos de usuário]
- [Documentação de resultados de [[Componentes/cards|card]] sorting ou tree testing] 