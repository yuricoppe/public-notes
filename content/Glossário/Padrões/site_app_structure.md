...
- **Navegação Principal (Primary Navigation):** Menu principal que dá acesso às seções de mais alto nível (ex: cabeçalho, menu lateral).
- **Navegação Secundária/Local (Secondary/Local Navigation):** Menus específicos para seções ou subseções.
- **Mapas do Site (Sitemaps):** Representação visual ou textual da hierarquia do conteúdo.
- **Breadcrumbs:** Indicam a localização atual do usuário na hierarquia do site.
- **Página Inicial (Homepage/Dashboard):** Ponto de partida principal, oferecendo acesso a seções chave e conteúdo em destaque.
- **Rodapé (Footer Navigation):** Links para informações secundárias, políticas, contato, etc.
...
### Faça
- Baseie a estrutura nos modelos mentais e necessidades dos seus usuários (resultado de pesquisa com usuários, card sorting, tree testing).
...
- O mapa do site, se fornecido, deve ser acessível.

## Exemplos / Cenários de Uso
- Estrutura de um portal de e-commerce (categorias de produtos, conta do usuário, carrinho, checkout).
- Estrutura de um portal de notícias (seções temáticas, artigos, busca, arquivos).
- Arquitetura de um aplicativo SaaS (dashboard, módulos de funcionalidades, configurações, ajuda).
...
- **Matricial:** Permite que os usuários naveguem por múltiplos atributos ou facetas (comum em filtros de busca).
