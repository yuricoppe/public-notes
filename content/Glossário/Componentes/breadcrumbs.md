...
3.  **Baseados em Atributo (Attribute-based):**
    *   Mostram atributos ou filtros selecionados para chegar a um conjunto de resultados (comum em e-commerce).
    *   Exemplo: `Home > Roupas > Camisetas > Tamanho: M > Cor: Azul`

## Melhores Práticas

-   **Posicionamento:** Geralmente no topo da área de conteúdo principal, abaixo do cabeçalho principal e acima do título da página.
-   **Separador:** Usar um separador visual claro entre os links (ex: `>`, `/`, `»`). O `>` é o mais comum e reconhecido.
-   **Link para Home:** O primeiro item deve ser quase sempre um link para a página inicial.
...
-   **Acessibilidade (a11y):**
    ...
    *   Usar uma lista ordenada (`<ol>`) ou não ordenada (`<ul>`) para os itens, pois representam uma sequência ou conjunto de links de navegação.
    *   Para o item da página atual que não é um link, usar `aria-current="page"`.

## Variações de Estilo

-   Tamanho da fonte.
-   Cor dos links e do texto da página atual.
-   Estilo do separador.
