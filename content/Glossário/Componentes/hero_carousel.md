...
O Hero Carousel (Carrossel de Destaque) é uma variação do componente Hero/Billboard que exibe múltiplos slides de destaque em rotação. Cada slide geralmente contém uma imagem ou vídeo, título, descrição e um Call to Action (CTA), similar a um Hero individual.
...
-   Mostrar múltiplas promoções ou mensagens importantes no topo de uma página inicial, quando há mais de uma mensagem principal competindo por atenção.
...
-   **(Opcional) Botão Play/Pause:** Se o carrossel tiver rotação automática.
...
-   **Acessibilidade (a11y):**
    ...
    *   **Pausar Rotação:** Garantir que usuários possam pausar, parar ou esconder o movimento se durar mais que 5 segundos (WCAG).
...
-   Usar um carrossel quando um simples grid ou uma lista de destaques seria mais eficaz. 