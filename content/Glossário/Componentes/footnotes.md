...
Footnotes (Notas de Rodapé) são referências ou informações adicionais colocadas na parte inferior de uma página (ou seção) para fornecer esclarecimentos, citações de fontes, ou comentários sobre um ponto específico no corpo do texto principal. Elas são indicadas no texto por um número sobrescrito, símbolo ou letra.
...
1.  **Marcador de Referência no Texto:** Um número (geralmente sobrescrito: ¹, ², ³), asterisco (*, **, ***), ou outra convenção de símbolo no corpo do texto, indicando a presença de uma nota de rodapé.
    *   Este marcador é geralmente um link que leva o usuário diretamente para a nota de rodapé correspondente.
...
3.  **Texto da Nota de Rodapé:** O conteúdo explicativo, citação ou comentário.
