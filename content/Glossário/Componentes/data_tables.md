...
-   [[Padrões/settings|Configurações]] de sistema que envolvem múltiplos itens.
-   Qualquer situação onde dados tabulares precisam ser apresentados de forma organizada.

## Funcionalidades Essenciais

-   **[[Elementos/cabecalhos|Cabeçalhos]] de Coluna (Headers):** Rótulos claros para cada coluna, indicando o tipo de dado que ela contém.
-   **Linhas de Dados (Rows):** Cada linha representa um item ou [[Padrões/create_account|registro]].
...
-   **Ordenação (Sorting):** Permitir que o usuário clique nos [[Elementos/cabecalhos|cabeçalhos]] das colunas para ordenar os dados em ordem ascendente ou descendente.
...
-   **Seleção de Linhas (Row Selection):** Permitir que o usuário selecione uma ou mais linhas para realizar ações em lote (ex: deletar, editar, exportar).
    *   [[Elementos/form_controls|Checkboxes]] para seleção múltipla.
-   **Ações por Linha (Row Actions):** [[Elementos/botoes|Botões]] ou [[Componentes/menu|menus]] de contexto em cada linha para ações específicas daquele item (ex: Editar, Ver Detalhes, Excluir).
...
-   **Legibilidade:** Usar [[Linguagem Visual/tipografia|tipografia]] clara, [[Linguagem Visual/espacamento|espaçamento]] adequado (padding nas células) e contraste suficiente.
-   **Escaneabilidade:** Facilitar a leitura rápida dos dados. Listras de zebra (alternar [[Linguagem Visual/cor|cor]] de fundo das linhas) podem ajudar.
-   **Responsividade:** Tabelas são notoriamente difíceis de tornar responsivas.
    *   **Abordagens Comuns:** Rolagem horizontal, colapsar colunas menos importantes em um [[Componentes/menu|menu]] "mais", transformar linhas em [[Componentes/cards|cards]] em telas pequenas, ou priorizar colunas.
...
-   **Acessibilidade (a11y):**
    ...
    *   Garantir que todas as funcionalidades interativas (ordenação, paginação, [[Componentes/filters|filtros]], seleções, ações) sejam acessíveis e operáveis por teclado.
    *   Indicar o estado de ordenação atual (ex: com um ícone e `aria-sort` no `<th>`).
-   **Clareza nos [[Elementos/cabecalhos|Cabeçalhos]]:** Devem ser concisos e descritivos.
