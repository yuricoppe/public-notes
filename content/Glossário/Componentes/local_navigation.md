...
-   **Abas (Tabs):** Para alternar entre diferentes painéis de conteúdo dentro da mesma página ou seção (ex: abas em uma página de perfil de usuário: "Visão Geral", "Atividade", "Configurações").
-   **Menus Laterais de Seção (Section Side Menus):** Em uma seção complexa de um site (ex: configurações de conta, documentação de produto), um menu lateral pode listar todas as subpáginas ou subseções.
-   **Menus Horizontais Secundários:** Uma barra de links abaixo do cabeçalho principal para navegar entre as principais áreas de uma seção específica.
-   **Breadcrumbs:** Embora também um auxílio de localização, podem funcionar como navegação local para níveis hierárquicos superiores.
-   **Menu de Steps (Passos):** Para guiar o usuário através de um processo linear com múltiplas etapas (ex: checkout, configuração).
...
1.  **Abas (Tabs):**
    *   **Lista de Abas:** Contêiner com os rótulos das abas clicáveis.
