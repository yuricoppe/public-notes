...
-   **Formulários Curtos:** (ex: login, cadastro rápido, adicionar um item simples).
-   **Visualização de Conteúdo Detalhado:** (ex: visualizar uma imagem em tamanho maior, detalhes de um produto sem sair da listagem).
-   **Notificações que Exigem Ação.**
-   **Seletores ou Pickers Complexos:** (ex: um seletor de arquivos avançado).

## Tipos Comuns

1.  **Modal de Alerta (Alert Dialog):** Informa o usuário sobre uma situação e geralmente tem apenas um botão de "OK" ou "Fechar".
2.  **Modal de Confirmação (Confirm Dialog):** Pede ao usuário para confirmar uma ação. Geralmente tem dois botões (ex: "Sim/Não", "Confirmar/Cancelar").
3.  **Modal de Prompt/Entrada (Prompt Dialog):** Solicita uma entrada de dados do usuário através de um ou mais campos de formulário.
...
-   **Conteúdo Principal:** A mensagem, formulário ou informação a ser apresentada.
-   **Botões de Ação:** (ex: Confirmar, Cancelar, Salvar, Fechar).
-   **Mecanismo de Fechamento:**
    *   Um botão "Fechar" explícito (geralmente um "X" no canto superior direito).
    *   Ação de um dos botões principais (ex: "Cancelar" fecha o dialog).
...
-   **Responsividade:** Dialogs devem se adaptar a diferentes tamanhos de tela, evitando barras de rolagem desnecessárias dentro do dialog e garantindo que os botões de ação estejam sempre visíveis.
