...
-   **[[Padrões/form_structure|Formulários]] Curtos:** (ex: [[Padrões/authentication|login]], cadastro rápido, adicionar um item simples).
-   **Visualização de Conteúdo Detalhado:** (ex: visualizar uma [[Elementos/imagem|imagem]] em tamanho maior, detalhes de um produto sem sair da listagem).
-   **Notificações que Exigem Ação.**
-   **Seletores ou Pickers Complexos:** (ex: um seletor de arquivos avançado).

## Tipos Comuns

1.  **Modal de Alerta (Alert Dialog):** Informa o usuário sobre uma situação e geralmente tem apenas um [[Elementos/botoes|botão]] de "OK" ou "Fechar".
2.  **Modal de Confirmação (Confirm Dialog):** Pede ao usuário para confirmar uma ação. Geralmente tem dois [[Elementos/botoes|botões]] (ex: "Sim/Não", "Confirmar/Cancelar").
3.  **Modal de Prompt/Entrada (Prompt Dialog):** Solicita uma entrada de dados do usuário através de um ou mais campos de [[Padrões/form_structure|formulário]].
...
-   **Conteúdo Principal:** A mensagem, [[Padrões/form_structure|formulário]] ou informação a ser apresentada.
-   **[[Elementos/botoes|Botões]] de Ação:** (ex: Confirmar, Cancelar, Salvar, Fechar).
-   **Mecanismo de Fechamento:**
    *   Um [[Elementos/botoes|botão]] "Fechar" explícito (geralmente um "X" no canto superior direito).
    *   Ação de um dos [[Elementos/botoes|botões]] principais (ex: "Cancelar" fecha o dialog).
...
-   **Responsividade:** Dialogs devem se adaptar a diferentes tamanhos de tela, evitando barras de rolagem desnecessárias dentro do dialog e garantindo que os [[Elementos/botoes|botões]] de ação estejam sempre visíveis.
