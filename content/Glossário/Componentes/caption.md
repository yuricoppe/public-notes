...
Captions, ou legendas, são textos descritivos curtos associados a outros elementos da interface, como imagens, tabelas, gráficos, ou grupos de campos de formulário. Elas fornecem contexto adicional, atribuição ou uma breve explicação sobre o elemento ao qual se referem.
...
-   **Figuras (Ilustrações, Blocos de Código):** Similar a imagens e tabelas.
-   **Grupos de Campos de Formulário:** Usar `<legend>` dentro de um `<fieldset>` para descrever um grupo de controles relacionados.
-   **Citações (Blockquotes):** Atribuir a autoria da citação.
...
-   **Distinção Visual:** Legendas devem ser visualmente distintas do texto principal e do elemento que acompanham (ex: tamanho de fonte menor, cor de texto diferente, itálico), mas ainda legíveis.
-   **Acessibilidade (a11y):**
    *   **Imagens:** Se a imagem já tem um `alt` text descritivo e a legenda apenas repete essa informação ou adiciona detalhes menores, certifique-se de que não haja redundância excessiva. Se a legenda fornece a descrição primária, o `alt` text pode ser mais conciso ou referenciar a legenda.
    ...
    *   **Figuras:** Utilizar os elementos `<figure>` e `<figcaption>` para associar semanticamente uma imagem, ilustração, diagrama, trecho de código, etc., com sua legenda.
        ...
        ```
    *   **Formulários:** Utilizar `<fieldset>` e `<legend>` para agrupar e nomear grupos de controles de formulário relacionados. Isso ajuda usuários de leitores de tela a entenderem o contexto dos campos.
        ```html
        <fieldset>
...
-   **Tipografia:** Geralmente menor que o corpo do texto (ex: 80-90% do tamanho do corpo do texto).
...
-   Legendas longas que se assemelham a parágrafos de texto.
