...
Filtros são componentes de interface que permitem aos usuários refinar e reduzir um conjunto de dados (como uma lista de produtos, resultados de busca, itens em uma tabela) com base em critérios específicos. Eles ajudam os usuários a encontrar o que procuram de forma mais rápida e eficiente.

## Casos de Uso

-   E-commerce: filtrar produtos por categoria, preço, marca, tamanho, cor, etc.
-   Resultados de Busca: refinar resultados por tipo de conteúdo, data, relevância.
-   Tabelas de Dados e Listagens: filtrar por status, data, categoria ou qualquer outro atributo dos itens.
-   Dashboards: filtrar dados de gráficos e visualizações.
-   Mapas: filtrar pontos de interesse.

## Tipos Comuns de Controles de Filtro

Filtros podem ser compostos por diversos elementos de UI:

-   **Checkboxes:** Para selecionar múltiplas opções de um atributo (ex: filtrar por várias marcas).
-   **Radio Buttons:** Para selecionar uma única opção de um atributo (ex: filtrar por "Em estoque" ou "Fora de estoque", se forem mutuamente exclusivos).
-   **Dropdowns/Selects:** Para selecionar uma ou múltiplas opções de uma lista, especialmente útil quando há muitas opções.
-   **Sliders (Deslizantes):** Para selecionar um intervalo de valores numéricos (ex: faixa de preço, avaliação).
-   **Campos de Texto/Busca:** Para filtrar por termos de pesquisa dentro de um atributo (ex: buscar nome do produto).
-   **Botões de Toggle/Chips:** Para aplicar ou remover rapidamente filtros de categorias comuns.
...
-   **Modal/Drawer:** Filtros são apresentados em um dialog ou drawer, especialmente em telas mobile para economizar espaço.
-   **Filtros Inline:** Opções de filtro diretamente incorporadas no conteúdo (ex: cabeçalhos de tabela clicáveis que também filtram).
