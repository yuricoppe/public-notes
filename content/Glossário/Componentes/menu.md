...
O componente Menu é uma lista de opções ou ações apresentadas ao usuário. Pode ser usado para navegação, executar comandos ou selecionar configurações. Esta documentação cobre o conceito genérico de menu, que pode se manifestar de várias formas (ex: dropdown menu, context menu, navigation menu).

## Casos de Uso

-   **Menu de Navegação Principal (Navbar Menu):** Parte da navegação global do site.
-   **Menu Dropdown:** Uma lista de opções que aparece quando um botão ou item de menu é clicado/hover.
...
-   **Menu de Ações de Item (Item Action Menu):** Um menu (frequentemente um ícone de "três pontos" ou "kebab") associado a um item em uma lista ou card, revelando ações para aquele item.
-   **Menu de Configurações de Conta/Perfil.**
-   **Menu de Filtros ou Ordenação.**

## Elementos Comuns

-   **Gatilho (Trigger):** O elemento que abre o menu (pode ser um botão, link, ícone, ou o próprio item de menu pai em um menu multinível).
